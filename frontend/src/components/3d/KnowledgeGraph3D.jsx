...
export function KnowledgeGraph3D({ onSelectNode, selectedNode, filter = 'All', graphData = { nodes: [], edges: [] } }) {
  const nodesWithPositions = useMemo(() => positionNodes(graphData.nodes || []), [graphData.nodes]);
  const links = graphData.edges || [];

  const filteredNodes = useMemo(() => {
    if (filter === 'All') return nodesWithPositions;
    return nodesWithPositions.filter(n => n.category === filter || n.type === filter);
  ...
  return (
    <div className="w-full h-full relative min-h-[500px]">
      ...
      >
        ...
        <GraphLinks nodes={nodesWithPositions} links={links} />
