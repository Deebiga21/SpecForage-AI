...
export function Sidebar() {
  ...
  const navItems = [
    ...
    { name: 'Insights', path: '/app/insights', icon: BrainCircuit },
  ...
  return (
    <aside className={`relative z-30 transition-all duration-300 bg-white rounded-2xl border border-black flex flex-col justify-between shadow-sm ${
      collapsed ? 'w-20' : 'w-64'
    } h-full p-4 shrink-0`}>
                  ...
                  <>
                    ...
                    {!collapsed && (
                      <div className="flex items-center justify-between w-full">
                        ...
                        {item.badge && (
                          <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 border border-black">
                            {item.badge}
                          </span>
                        )}
                      </div>
      ...
      {/* Sidebar Footer Card */}
      {!collapsed && (
        <div className="p-4 rounded-xl bg-slate-50 border border-black text-xs shadow-sm">
