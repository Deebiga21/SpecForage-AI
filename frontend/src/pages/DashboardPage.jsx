...
export function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 justify-items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          ...
          <div className="space-y-3">
            {stats?.recent_jobs?.map((job, idx) => (
              <motion.div key={job.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }}
                className="p-3.5 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-between shadow-sm"
              >
              ...
              {filteredProducts.map((p) => (
                <tr key={p.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="py-3.5 px-3 text-right">
                    <button onClick={() => navigate(`/app/products/${p.id}`)} className="px-3 py-1.5 rounded-lg bg-white border border-slate-200 shadow-sm hover:bg-blue-50 text-slate-700 hover:text-blue-700 font-medium transition-colors inline-flex items-center gap-1.5">
