...
import { ReviewPanel } from '../components/widgets/ReviewPanel';
...
export function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 justify-items-center bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
          ...
          <div className="space-y-3">
            {stats?.recent_jobs?.map((job, idx) => (
              <motion.div key={job.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }}
                className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 flex items-center justify-between shadow-sm"
              >
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg bg-white dark:bg-slate-900 shadow-sm flex items-center justify-center border border-slate-200 dark:border-slate-700">
                    <FileText className="w-4 h-4 text-blue-600" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs font-bold text-slate-900 dark:text-white truncate max-w-[200px]">{job.filename}</span>
                    </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {job.result_product_id && (
                     <button onClick={() => navigate(`/app/products/${job.result_product_id}`)} className="p-1 rounded hover:bg-slate-200">
                        <ArrowUpRight className="w-4 h-4 text-slate-500 dark:text-slate-400 hover:text-blue-600" />
                     </button>
                  )}
                </div>
              </motion.div>
            ))}
            {stats?.recent_jobs?.length === 0 && <div className="text-sm text-slate-500 dark:text-slate-400 text-center py-4">No recent jobs</div>}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-5">
           <div className="pb-3 border-b border-slate-100 dark:border-slate-800">
            <h3 className="font-display font-semibold text-sm text-slate-900 dark:text-white">System Metrics</h3>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-mono">Overall database health</p>
          </div>
          <div className="space-y-4">
             <div>
              <div className="flex justify-between text-xs font-mono mb-1.5"><span className="text-slate-600 dark:text-slate-300">Extraction Confidence</span><span className="text-blue-700 font-bold">~90%</span></div>
             ...
             <div>
              <div className="flex justify-between text-xs font-mono mb-1.5"><span className="text-slate-600 dark:text-slate-300">Taxonomy Mapping Precision</span><span className="text-emerald-600 font-bold">~95%</span></div>
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-3 border-b border-slate-100 dark:border-slate-800">
          <div>
            <h3 className="font-display font-semibold text-base text-slate-900 dark:text-white">Recent Product Records</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 font-mono">Traceable industrial schemas & verified records</p>
          </div>
          <div className="flex items-center gap-2">
            {['All', 'Verified', 'Review'].map(f => (
              <button key={f} onClick={() => setSelectedFilter(f)} className={`px-3 py-1.5 rounded-xl text-xs font-mono font-medium transition-all ${selectedFilter === f ? 'bg-blue-600 text-white shadow-sm' : 'bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-slate-900 dark:text-white'}`}>{f}</button>
            ))}
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs font-sans">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 font-mono text-[11px] uppercase tracking-wider">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filteredProducts.map((p) => (
                <motion.tr layoutId={`product-row-${p.id}`} key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 dark:bg-slate-800 transition-colors group">
                  <td className="py-3.5 px-3">
                    <div className="font-bold font-mono text-slate-900 dark:text-white">{p.sku || "N/A"}</div>
                    <div className="text-slate-500 dark:text-slate-400 text-[11px] truncate max-w-xs">{p.product_name}</div>
                  </td>
                  <td className="py-3.5 px-3 text-slate-600 dark:text-slate-300 font-mono text-[11px]">{p.category || 'Uncategorized'}</td>
                  <td className="py-3.5 px-3">
                  ...
                  <td className="py-3.5 px-3 text-right">
                    {p.review_status === 'pending' ? (
                      <button onClick={() => setReviewProduct(p)} className="px-3 py-1.5 rounded-lg bg-blue-50 border border-blue-200 shadow-sm hover:bg-blue-100 text-blue-700 font-medium transition-colors inline-flex items-center gap-1.5">
                        <AlertTriangle className="w-3.5 h-3.5" /><span>Review</span>
                      </button>
                    ) : (
                      <button onClick={() => navigate(`/app/products/${p.id}`)} className="px-3 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-800 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:text-blue-700 font-medium transition-colors inline-flex items-center gap-1.5">
              ...
              {filteredProducts.length === 0 && <tr><td colSpan="5" className="py-4 text-center text-slate-500 dark:text-slate-400">No products found.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <ReviewPanel
        ...
        layoutId={reviewProduct ? `product-row-${reviewProduct.id}` : null}
      />
    </div>
  );
}
