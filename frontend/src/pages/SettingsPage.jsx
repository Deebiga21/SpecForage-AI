...
export function SettingsPage() {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-black">
        <div>
      ...
      {Object.keys(settings).map(category => (
         <div key={category} className="bg-white p-6 rounded-2xl shadow-sm border border-black space-y-4">
                  ...
                  <div key={setting.key} className="flex justify-between items-center py-2">
                     ...
                     <div className="w-1/3 text-right">
                        {setting.type === 'boolean' ? (
                           ...
                           <input type="number" step={setting.type === 'float' ? '0.01' : '1'} value={setting.value} onChange={e => handleUpdate(setting.key, e.target.value)} className="w-full p-2 bg-slate-50 border border-black rounded text-sm text-slate-900 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                        ) : (
                           <input type="text" value={setting.value} onChange={e => handleUpdate(setting.key, e.target.value)} className="w-full p-2 bg-slate-50 border border-black rounded text-sm text-slate-900 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                        )}
                     </div>
                  </div>
               ))}
