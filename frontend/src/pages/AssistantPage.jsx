...
export function AssistantPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] gap-6 w-full max-w-[1600px] mx-auto">
      ...
      <div className="hidden lg:flex w-64 shrink-0 flex-col bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 bg-slate-50 font-display font-bold text-sm text-slate-900">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-white">
          {messages.map((msg, idx) => (
             <div key={idx} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 mt-1 shadow-sm ${
                   msg.role === 'user' ? 'bg-blue-600 text-white' : 'bg-slate-50 text-blue-600 border border-slate-200'
                }`}>
                   {msg.role === 'user' ? <User className="w-4 h-4" /> : <Bot className="w-4 h-4" />}
                </div>
                <div className={`max-w-[85%] rounded-2xl p-4 text-[14px] leading-relaxed shadow-sm ${
                   msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-sm' : 'bg-slate-50 text-slate-800 border border-slate-200 rounded-tl-sm'
                }`}>
          ...
          {loading && (
             <div className="flex gap-4">
                <div className="w-8 h-8 rounded-full bg-slate-50 text-blue-600 border border-slate-200 flex items-center justify-center shrink-0 shadow-sm">
                   <Bot className="w-4 h-4" />
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-2xl rounded-tl-sm p-4 flex items-center gap-3 shadow-sm">
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50 shrink-0">
          <div className="relative w-full flex items-center shadow-sm">
             <input 
               ...
               className="w-full bg-white border border-slate-200 rounded-xl py-3 pl-4 pr-12 text-[14px] text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-shadow"
               disabled={loading}
             />
             <button 
               onClick={handleSend}
      ...
      <div className="hidden xl:flex w-80 shrink-0 flex-col bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 bg-slate-50 font-display font-bold text-sm text-slate-900">
          Contextual Evidence
        </div>
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="p-3 border border-slate-200 rounded-xl text-xs space-y-2 text-slate-600 italic text-center">
