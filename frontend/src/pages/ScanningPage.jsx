    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-black">
        ...
        {isCompleted && (
          <button
            ...
            className="px-4 py-2 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-700 text-xs font-medium font-mono flex items-center gap-2 border border-black transition-colors shadow-sm"
          >
      ...
      {!uploadedFile ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 bg-white p-5 rounded-2xl border border-black shadow-sm space-y-4 relative overflow-hidden">
            ...
            <div className="relative aspect-[3/4] rounded-xl bg-slate-50 border border-black overflow-hidden flex flex-col justify-center items-center font-mono text-xs text-slate-500 select-none shadow-inner">
                ...
                {jobState?.steps?.map((step) => {
                  ...
                  return (
                    <div key={step.id} className={`p-3 rounded-xl border transition-all duration-300 flex items-start gap-3 shadow-sm ${
                        ...
                        : isDone ? 'bg-white border-black text-slate-700'
                        : isErr ? 'bg-rose-50 border-rose-200 text-rose-700'
                        : 'bg-slate-50 border-black text-slate-400 opacity-70'
                    }`}>
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5 shadow-sm ${
                        ...
                        : 'bg-white text-slate-400 border border-black'
                      }`}>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-6 rounded-2xl border-2 border-emerald-400 shadow-md space-y-4">
                  ...
                  <div className="space-y-3">
                    <div className="pt-3 flex items-center gap-3">
                      <button onClick={() => navigate(`/app/products/${jobState.result_product_id}`)} className="flex-1 py-2.5 rounded-xl bg-slate-50 border border-black text-slate-800 font-semibold text-xs flex items-center justify-center gap-2 hover:bg-slate-100 transition-colors shadow-sm">
                        View Product Record
                      </button>
