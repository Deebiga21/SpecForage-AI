    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/app/products')} className="p-2 rounded-xl bg-slate-50 border border-slate-200 hover:bg-slate-100 text-slate-600 transition-colors">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <h1 className="text-2xl font-bold font-display text-slate-900">{product.product_name}</h1>
        ...
        <div className={`px-3 py-1.5 rounded-lg border font-mono text-sm font-bold shadow-sm ${
           product.confidence >= 0.9 ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'
        }`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <h3 className="font-display font-semibold text-lg text-slate-900 mb-4 pb-2 border-b border-slate-100 flex items-center gap-2">
               <FileText className="w-5 h-5 text-blue-600"/> Product Summary & Attributes
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {specs.map(s => (
                <div key={s.id} className="p-3 bg-slate-50 rounded-xl border border-slate-100 flex justify-between items-center hover:border-blue-200 transition-colors">
                  <span className="text-xs text-slate-500 font-mono">{s.field}</span>
                  <span className="text-sm font-mono font-bold text-slate-800">{s.value} {s.unit}</span>
                </div>
              ))}
              {specs.length === 0 && <div className="col-span-full text-slate-500 text-sm italic">No technical attributes extracted.</div>}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            ...
            <div className="space-y-4">
               <div>
                  <span className="text-xs text-slate-500 font-mono block mb-1">Category / Taxonomy</span>
                  <span className="text-sm font-bold text-slate-800">{product.category || 'Uncategorized'}</span>
               </div>
               <div>
                  <span className="text-xs text-slate-500 font-mono block mb-1">Validation Status</span>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            ...
            {compliance.length === 0 ? <p className="text-sm text-slate-500 italic">No compliance data.</p> : (
               <div className="space-y-3">
                  {compliance.map(c => (
                     <div key={c.id} className="flex justify-between items-center p-2 bg-slate-50 rounded-lg border border-slate-100">
                        <span className="text-xs font-mono text-slate-700">{c.standard}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    {specs.map(s => {
                       ...
                       return (
                          <tr key={s.id} className="hover:bg-slate-50">
                             <td className="py-3 pr-2 font-mono text-slate-600">{s.field}</td>
                             <td className="py-3 pr-2 font-bold text-slate-900">{s.value} {s.unit}</td>
                             <td className="py-3 pr-2">
                                <span className={`px-1.5 py-0.5 rounded font-mono ${s.confidence > 0.9 ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
         ...
         <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
           ...
           {relationships.length === 0 ? <p className="text-sm text-slate-500 italic">No verified relationships found in Knowledge Graph.</p> : (
               <div className="space-y-3">
                  {relationships.map(r => (
                     <div key={r.id} className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex justify-between items-center cursor-pointer hover:border-blue-400 transition-colors shadow-sm" onClick={() => navigate(`/app/products/${r.related_product.id}`)}>
                        <div>
                           <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="text-xs text-slate-500 font-mono">Conf: {Math.round(r.confidence * 100)}%</span>
                           </div>
                           <span className="text-sm font-semibold text-slate-900">{r.related_product.name} (SKU: {r.related_product.sku})</span>
                        </div>
